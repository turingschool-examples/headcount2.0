...
describe('App', () => {
  ...
  it('should update state when clearComparison is invoked', () => {
    const initialCompareData = [{location: 'denver', stats: [{2001: 2}], selected: true},
      {location: 'boulder', stats: [{2002: 1}], selected: true}];
    const initailAnalysis = {'AGATE 300': 0.909, 'ARICKAREE R-2': 0.739, compared: 1.23}

    wrapper.setState({ compareData: initialCompareData, analysis: initailAnalysis })
  ...
  it('should add a district to compareData and invoke makeAnalysis', () => {
    const initialState = []
    const mockDistrict = {location: 'Denver', stats: [{2007: 1}]}
    const expected = [{location: 'Denver', stats: [{2007: 1}], selected: true}]
  ...
  it('should remove a card from compareData', () => {
    const initialCompareData = [{location: 'denver', stats: [{2001: 2}], selected: true},
    {location: 'boulder', stats: [{2002: 1}], selected: true}];
    const selectedCard = {location: 'boulder', stats: [{2002: 1}], selected: true}
    const expected = [{location: 'denver', stats: [{2001: 2}], selected: true}]
