export default class DistrictRepository {
  ...
  cleanData = (stats) => {
    return stats.reduce((acc, district) => {
      ...
      if (!acc[district.Location.toUpperCase()]){
        ...
        Object.assign( acc[district.Location.toUpperCase()], 
          {[district.TimeFrame]: roundedData});  
      }
      return acc; 
    }, {});
  ...
  compareDistrictAverages = (str1, str2) => {
    ...
    let location1 = Object.keys(this.stats).filter(stat => 
      stat.toUpperCase() === str1.toUpperCase()); 
    let location2 = Object.keys(this.stats).filter(stat => 
      stat.toUpperCase() === str2.toUpperCase());
    return { [location1]: avrg1, [location2]: avrg2, 
      'compared':(Math.round((avrg1/avrg2) * 1000) / 1000) };
  }
